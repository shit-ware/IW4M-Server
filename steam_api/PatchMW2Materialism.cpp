...
Material* Material_Register(const char* filename)
{
	...
	if (!_stricmp(filename, "preview_mp_crash_trop"))
	{
		return Material_Obtain(filename, filename);
	}

	if (!_stricmp(filename, "preview_mp_storm_spring"))
	{
		return Material_Obtain(filename, filename);
	}

	if (!_stricmp(filename, "preview_mp_estate_tropical"))
	{
		return Material_Obtain(filename, filename);
	}

	if (!_stricmp(filename, "preview_mp_fav_tropical"))
	{
		return Material_Obtain(filename, filename);
	}
