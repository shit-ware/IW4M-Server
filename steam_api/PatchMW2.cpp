...
void PatchMW2_159()
{
	...
	*(BYTE*)0x45114C = 0xEB;
	...
	// master server
	strcpy((char*)0x6D9CBC, NP_SERVER);
	...
	*(const char**)0x51F5A6 = "Could not load image '%s'. If this image name starts with 'preview_mp_', you might be missing iw_22.iwd/iw_23.iwd. Read the relevant FAQ entry on http://repziw4.de :)";
...
void HelloIW(int type)
{
	...
	Com_Printf(type, "http://repziw4.de");
...
void CL_OpenStore_f()
{
	ShellExecute(NULL, "open", "http://status.repziw4.de/downloads/DLC.rar", 0, 0, SW_SHOWNORMAL);
	Auth_OpenDLCStore();
}
