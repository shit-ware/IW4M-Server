...
void InitDedicatedVars() {
	...
	*(dvar_t**)0x62E279C = Dvar_RegisterString("ui_mapname", "", DVAR_FLAG_NONE, "Current map name");
	...
	sv_rconPassword = (dvar_t*)Dvar_RegisterString("rcon_password", "", DVAR_FLAG_NONE, "[IW4] the password for rcon");
	rcon_ip = (dvar_t*)Dvar_RegisterString("rcon_ip", "", DVAR_FLAG_NONE, "[IW4] the only ip address we allow access to rcon (Leave empty if disabled)");
	...
	iw4m_me = (dvar_t*)Dvar_RegisterInt("iw4m_me", 1, 0, 1, DVAR_FLAG_SERVERINFO, "Allow or disable '/me' command");

	sv_master[0] = Dvar_RegisterString("sv_master1", "server.repziw4.de", DVAR_FLAG_NONE, "");
...
void __declspec(naked) GSayPreHookFunc()
{
...
#endif
	{
		...
		if (!_strnicmp(&gspText[1], "/me ", 4) && gspText[5] != '\0')
		{
			...
			if (iw4m_me && iw4m_me->current.boolean)
			{
...
void OnClientDisconnectHookFunc(client_t* client)
{
	...
	Auth_OnDisconnect(client - svs_clients);
}
...
void PatchMW2_Dedicated()
{
	...
	*(BYTE*)0x60B240 = 0xC3;
	...
	onClientDisconnectHook.initialize(onClientDisconnectHookLoc, OnClientDisconnectHookStub);
	onClientDisconnectHook.installHook();
...
void SV_MasterHeartbeat() {
	...
	for (int i = 0; i < 6; i++)
	{
		...
		char heartbeat[255];
		_snprintf(heartbeat, sizeof(heartbeat), "%s;%s", "IW4", NP_ReturnLicenseKey());
