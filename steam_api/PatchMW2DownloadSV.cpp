...
void SV_cp(void* client)
{
	Com_Printf(0, "Denying request CP from %i: %s\n", CLIENTNUM(client), Cmd_ArgvSV(1));
}

void SV_userinfo(void* client)
{
	Com_Printf(0, "Denying request userinfo from %i: %s\n", CLIENTNUM(client), Cmd_ArgvSV(1));
}

void SV_vdr(void* client)
{
	Com_Printf(0, "Denying request vdr from %i: %s\n", CLIENTNUM(client), Cmd_ArgvSV(1));
}

void SV_matchdatadone(void* client)
{
	Com_Printf(0, "Denying request matchdatadone from %i: %s\n", CLIENTNUM(client), Cmd_ArgvSV(1));
}

void SV_status(void* client)
{
	Com_Printf(0, "Denying request status from %i: %s\n", CLIENTNUM(client), Cmd_ArgvSV(1));
}

void SV_nulled(void* client)
{
...
{
	// move ucmds
	memcpy(&ucmds, (void*)0x79B9E8, sizeof(ucmd_t) * 8);
	...
	*(DWORD*)0x62596C = (DWORD)ucmds;
	...
	// Kill some requests for security reasons
	ucmds[7].name = "status";
	ucmds[7].func = SV_nulled;
	ucmds[7].flag = 0;
	
	for (int i = 0; i < 8; i++) {
		//if (!strcmp(ucmds[i].name, "cp"))
		//	ucmds[i].func = SV_nulled;
		if (!strcmp(ucmds[i].name, "userinfo"))
			ucmds[i].func = SV_nulled;
		//if (!strcmp(ucmds[i].name, "vdr"))
		//	ucmds[i].func = SV_nulled;
		//if (!strcmp(ucmds[i].name, "matchdatadone"))
		//	ucmds[i].func = SV_matchdatadone;
	}
