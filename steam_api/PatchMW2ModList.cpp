...
void ModList_UIScript_LoadMods(char* args)
{
	...
	modInfo.mods = Sys_ListFiles((char*)va("%s\\%s", (*fs_homepath)->current.string, "mods"), NULL, NULL, &modInfo.max, NULL);
	//modInfo.mods = Sys_ListFiles((char*)va("%s\\%s", (*fs_homepath)->current.string), "ff", 0, &modInfo.max, 0);
	////char** modfiles = Sys_ListFiles((char*)va(path, (*fs_basepath)->current.string), "ff", 0, &num, 0);
}
