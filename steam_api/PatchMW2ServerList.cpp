...
void CL_GlobalServers_f( void ) {
	...
	NET_StringToAdr("server.repziw4.de", &dpServer);
	cls.numglobalservers = -1;
	cls.pingUpdateSource = 0;
	dpServer.type = NA_IP;
	dpServer.port = htons(20810);
	...
	NET_OutOfBandPrint(NS_SERVER, dpServer, command);
	...
	if (server) {
		if (info) {
			...
			if (server->mod[0])
			{
				...
				if (_stricmp(UI_LocalizeGameType(server->gameType), server->gameType) != 0)
				{
...
char* UI_LocalizeGameType(char* gameType)
{
	...
	if (_stricmp(gameType, "killcon") == 0)
	{
		return "Kill Confirmed";
	}
