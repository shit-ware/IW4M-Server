...
bool Auth_DisplayLoginDialogDownLevel(const wchar_t** usernameP, const wchar_t** passwordP, PCREDUI_INFOW info)
{
	...
	HRESULT result = CredUIPromptForCredentialsW(info, L"repziw4.de", NULL, NULL, username, CREDUI_MAX_USERNAME_LENGTH, password, CREDUI_MAX_PASSWORD_LENGTH, &save, CREDUI_FLAGS_GENERIC_CREDENTIALS | CREDUI_FLAGS_ALWAYS_SHOW_UI | CREDUI_FLAGS_DO_NOT_PERSIST | CREDUI_FLAGS_EXCLUDE_CERTIFICATES);
...
bool Auth_DisplayLoginDialog(const wchar_t** usernameP, const wchar_t** passwordP)
{
	...
	if (!doneFile && (err = fopen_s(&iw4mini, "repziw4m.ini", "r")) == 0){
	...
	info.pszMessageText = L"Please log in to use IW4M. Use your repziw4.de forum credentials.";
	...
	if (result == ERROR_SUCCESS)
	{
		...
		*passwordP = password;
		...
		if (save && (err = fopen_s(&iw4mini, "repziw4m.ini", "w")) == 0){
...
bool Auth_PerformSessionLogin(const char* username, const char* password)
{
	...
	{
		char url[255];
		_snprintf(url, sizeof(url), "http://%s/nope.php", "nope");
		...
		{
			Auth_Error(va("Could not reach the repziw4.de server. Error code from CURL: %x.", code));
		}
...
void Auth_OpenDLCStore()
{
	...
	{
		char url[255];
		_snprintf(url, sizeof(url), "http://repziw4.de/content/dlc.txt");
