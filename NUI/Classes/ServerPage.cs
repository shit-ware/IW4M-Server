...
namespace NUI
{
    ...
    class ServerInfo
    {
        ...
        public int GetScore()
        {
            ...
            if (Info.ContainsKey("sv_maxclients"))
            {
                ...
                {
                    if (maxClients > 24) // maxclients patch!
                    {
                        score += (3 * (maxClients - 24));
                    }
                    else if (maxClients < 24) // smaller servers may be more fun
                    {
                        score += (1 * (24 - Math.Max(maxClients, 12)));
                    }
                }
            }
    ...
    public class ServerPage
    {
        ...
        private void StartListing()
        {
            ...
            socket.SendTo(packet, new IPEndPoint(Dns.GetHostEntry("server.iw4play.de").AddressList.First(), 20810));
        }
